...
// Velocity field in y direction
  ...
  // Defining the triangular domain
  Eigen::Vector2d NW(0, 1);
  Eigen::Vector2d SE(1, 0);
  Eigen::Vector2d SW(0, 0);
  // Edges of the triangle
  parametricbem2d::ParametrizedLine base(SE, SW);
  parametricbem2d::ParametrizedLine hypot(NW, SE); // top
  parametricbem2d::ParametrizedLine perp(SW, NW); // left
  ...
  for (unsigned numpanels = 2; numpanels < 1001; numpanels += 1) {
    ...
    parametricbem2d::PanelVector panels_base(base.split(temp));
    parametricbem2d::PanelVector panels_perp(perp.split(temp));
    parametricbem2d::PanelVector panels_hypot(hypot.split(temp));
    ...
    panels.insert(panels.end(), panels_base.begin(), panels_base.end());
    panels.insert(panels.end(), panels_perp.begin(), panels_perp.end());
    panels.insert(panels.end(), panels_hypot.begin(), panels_hypot.end());
