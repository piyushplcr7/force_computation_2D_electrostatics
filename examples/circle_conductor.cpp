...
// Velocity field in y direction
  ...
  // Defining the inner circular domain
  parametricbem2d::ParametrizedCircularArc circle(Eigen::Vector2d(0.2,0.3),1,2*M_PI,0);
  ...
  for (unsigned numpanels = 2; numpanels < 1001; numpanels += 1) {
    ...
    // Getting panels for the inner circle
    parametricbem2d::PanelVector panels_circ(circle.split(temp));
    ...
    panels.insert(panels.end(), panels_circ.begin(), panels_circ.end());
