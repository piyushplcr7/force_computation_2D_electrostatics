...
/*
 * Class to specify the dirichlet data in a non smooth way
 */
class G_NS{
public:
  double operator()(const Eigen::Vector2d &X) const {
    ...
    if (abs(x) > 2.99 || abs(y) > 2.99)
      ...
      return 1; // Explicitly giving the value of g on the boundary
  }
  ...
  Eigen::Vector2d grad(const Eigen::Vector2d &X) const {
    ...
    if (abs(x) > 2.99 || abs(y) > 2.99)
      ...
      // Since the point of evaluation is always going to be on the bdry
      double c = 1./3; // Cut off length
      auto f = [&](double r) {
        return 2./3 * std::pow(r,-1./3) * (1 - r*r/c*c);
      };
      // Checking if point near a corner
      if ((X-Eigen::Vector2d(0,0)).norm() < c) {
        // Distance from the corner
        double r = (X-Eigen::Vector2d(0,0)).norm();
        if (x < 1e-14)
          return Eigen::Vector2d(-f(r),0);
        else //if (y < 1e-14)
          return Eigen::Vector2d(0,-f(r));
      }
      else if ((X-Eigen::Vector2d(1,0)).norm() < c) {
        // Distance from the corner
        double r = (X-Eigen::Vector2d(1,0)).norm();
        if (1-x < 1e-14)
          return Eigen::Vector2d(f(r),0);
        else //if (y < 1e-14)
          return Eigen::Vector2d(0,-f(r));
      }
      else if ((X-Eigen::Vector2d(1,1)).norm() < c) {
        // Distance from the corner
        double r = (X-Eigen::Vector2d(1,1)).norm();
        if (1-x < 1e-14)
          return Eigen::Vector2d(f(r),0);
        else //if (1-y < 1e-14)
          return Eigen::Vector2d(0,f(r));
      }
      else if ((X-Eigen::Vector2d(0,1)).norm() < c) {
        // Distance from the corner
        double r = (X-Eigen::Vector2d(0,1)).norm() ;
        if (x < 1e-14)
          return Eigen::Vector2d(-f(r),0);
        else //if (1-y < 1e-14)
          return Eigen::Vector2d(0,f(r));
